...
function bench(maxIterations, iter) {
    ...
    while (iterations--) {
        ...
        // Re-create the references to pretend it's new data
...
bench(1, function parse_cold() {
    out = JSON.parse(json);
});


bench(1, function serialize_cold() {
    out = JSON.parse(json);
});

bench(100000, function parse_hot() {
    out = JSON.parse(json);
});

bench(100000, function serialize_hot() {
